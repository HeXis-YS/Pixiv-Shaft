...
class FragmentUserIllust : NetListFragment<FragmentBaseListBinding, ListIllust, IllustsBean>() {
    private var userID = 0
    private var showToolbar = false

    companion object {
        @JvmStatic
        fun newInstance(userID: Int, paramShowToolbar: Boolean): FragmentUserIllust {
            val args = Bundle()
            args.putInt(Params.USER_ID, userID)
            args.putBoolean(Params.FLAG, paramShowToolbar)
            val fragment = FragmentUserIllust()
    ...
    override fun repository(): RemoteRepo<ListIllust> = UserIllustRepo(userID)
    ...
    override fun initView() {
        ...
        baseBind.toolbar.setOnMenuItemClickListener { item: MenuItem ->
            if (item.itemId == R.id.action_bookmark) {
                ...
                entity.uuid = userID.toString() + "插画作品"
                entity.isShowToolbar = showToolbar
                entity.dataType = "插画作品"
    ...
    override fun getToolbarTitle(): String =
        if (showToolbar) getString(R.string.string_246) else super.getToolbarTitle()
